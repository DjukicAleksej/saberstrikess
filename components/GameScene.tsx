...
const GameScene: React.FC<GameSceneProps> = ({
  ...
  useEffect(() => {
    if (gameStatus === GameStatus.PLAYING) {
      console.log("Game started: Resetting state");
      // Reset local state
      setNotesState(chart.map(n => ({ ...n, hit: false, missed: false }))); // efficient deep copy for reset
      setCurrentTime(0);
  ...
  useFrame((state, delta) => {
    // --- Beat Pulsing & Audio Reactivity ---
    let bass = 0;
    let high = 0;

    if (gameStatus === GameStatus.PLAYING && analyser) {
      analyser.getByteFrequencyData(dataArray);
      // Bass: ~20-100Hz (Lower bins)
      bass = dataArray[2] / 255.0;
      // High: ~skipping to mid-highs
      high = dataArray[40] / 255.0;
    } else if (gameStatus === GameStatus.PLAYING && audioRef.current) {
      // Fallback to fake pulse if analyser fails (e.g. CORS issue on default song sometimes)
      const time = audioRef.current.currentTime;
      const beatPhase = (time % BEAT_TIME) / BEAT_TIME;
      bass = Math.pow(1 - beatPhase, 4); // Fake beats
    }

    if (ambientLightRef.current) {
      ambientLightRef.current.intensity = 0.1 + (bass * 0.5);
    }
    if (spotLightRef.current) {
      // Reactive Spot Light
      spotLightRef.current.intensity = 1.0 + (bass * 4.0);
      const colorShift = new THREE.Color().setHSL(0.6 + (high * 0.2), 1, 0.5);
      spotLightRef.current.color.lerp(colorShift, 0.1);
    }
