...
import { Extrude, Tetrahedron } from '@react-three/drei';
...
const Debris: React.FC<{ data: NoteData, timeSinceHit: number, color: string }> = ({ data, timeSinceHit, color }) => {
    ...
    useFrame(() => {
        ...
        if (flashRef.current) {
            ...
            if (timeSinceHit < flashDuration) {
                ...
                flashRef.current.scale.setScalar(1 + t * 2);
    ...
    const Shard = ({ offsetDir, moveDir, scale = 1 }: { offsetDir: number[], moveDir: number[], scale?: number }) => {
        ...
        return (
            <Tetrahedron ref={meshRef} args={[NOTE_SIZE * 0.4 * scale]} position={[offsetDir[0], offsetDir[1], offsetDir[2]]}>
    ...
    return (
        <group ref={groupRef}>
            ...
            {/* Shattered Pieces - Reduced count for optimizations */}
            <Shard offsetDir={[0, 0.2, 0]} moveDir={[0, 1.5, -0.5]} scale={0.6} />
            <Shard offsetDir={[0, -0.2, 0]} moveDir={[0, -1.5, -0.5]} scale={0.6} />

            {/* Center core shards */}
            <Shard offsetDir={[0, 0, 0]} moveDir={[0, 0, 1]} scale={0.4} />
        </group>
    );
};
